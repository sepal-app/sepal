...
|----------|-------------|
| `COOKIE_SECRET` | Master secret. The cookie key and the token secret are derived from it - min 16 chars |
| `TOKEN_SECRET` | Only used when running from the REPL; `-main` derives the token secret from `COOKIE_SECRET` - min 16 chars |
