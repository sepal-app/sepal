...
(defn bean
  "Bean icon from Lucide. Propagation — the propagule you start from.

  A bean rather than a seed-specific glyph because every method starts from
  one: a seed, a cutting, a division. The rail's other plant icons read the
  same way, Taxa's trees standing for every taxon and Material's sprout for
  every stage.

  One compact diagonal body with the eye picked out above it, against Taxa's
  two vertical trunks and Material's low, wide sprout with its ground line.
  Two large forms, so it survives the rail's 20px."
  [& {:keys [size] :or {size 24}}]
  [:svg {:xmlns "http://www.w3.org/2000/svg"
         :viewBox "0 0 24 24"
         :fill "none"
