   ...
   (for [{:keys [text category]} messages]
     (let [color (condp = category
       ...
       [:div {:class (html/attr color :w-full :pointer-events-auto :flex :items-center
                                :justify-between :gap-x-6  :px-6 :py-2.5
                                :sm:rounded-xl :sm:py-3 :sm:pl-4 :sm:pr-3.5
                                "min-w-[33%]" :text-center)
              :x-data "{show: true}"
              :x-show "show"}
        [:p {:class (html/attr :text-sm :leading-6 :text-white)}
         [:a {:href "#"}
          [:strong {:class (html/attr :font-semibold)}
           text]]]
        [:button {:type "button"
                  :class (html/attr :-m-1.5 :flex-none :p-1.5)
                  :x-on:click "show=false"}
         [:span {:class (html/attr :sr-only)} "Dismiss"]
