...
(defmethod ig/init-key ::app [_ {:keys [ring-defaults context cookie-secret _routes]}]
  (let [ring-defaults (-> (case ring-defaults
        ...
        router-options {:exception pretty/exception
        ...
        default-handler (reitit.ring/create-default-handler error-handlers)
        router (reitit.ring/router routes router-options)]
    (reitit.ring/ring-handler router default-handler)))
