...
create trigger organization_user_update_updated_at
  ...
  for each row execute procedure moddatetime (updated_at);
--;;
create or replace function is_organization_member (organization_id integer, user_id uuid)
  returns boolean
  language sql
  immutable
  as $$
...
$$;
--;;
create or replace function has_organization_role (organization_id integer, user_id uuid, roles organization_role[])
  returns boolean
  language sql
  immutable
  as $$
