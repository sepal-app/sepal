(ns sepal.app.routes.location.delete-test
  (:require [clojure.test :refer [deftest is testing use-fixtures]]
            [integrant.core :as ig]
            [next.jdbc.sql :as jdbc.sql]
            [peridot.core :as peri]
            [sepal.accession.interface :as accession.i]
            [sepal.app.test :as app.test]
            [sepal.app.test.fixtures :as tf]
            [sepal.app.test.system :refer [*db* default-system-fixture]]
            [sepal.contact.interface :as contact.i]
            [sepal.location.interface :as location.i]
            [sepal.material.interface :as material.i]
            [sepal.taxon.interface :as taxon.i]
            [sepal.test.interface :as test.i]
            [sepal.user.interface :as user.i])
  (:import [org.jsoup Jsoup]))

(use-fixtures :once default-system-fixture)

(defn- fixtures
  ([] (fixtures :editor))
  ([role]
   {[::user.i/factory :key/user] {:db *db*
                                  :password "testpassword123"
                                  :role role}
    [::location.i/factory :key/location] {:db *db*}
    [::contact.i/factory :key/contact] {:db *db*}
    [::taxon.i/factory :key/taxon] {:db *db*}
    [::accession.i/factory :key/accession] {:db *db*
                                            :taxon (ig/ref :key/taxon)
                                            :contact (ig/ref :key/contact)}}))

(defn- delete-url [location]
  (str "/location/" (:location/id location) "/delete/"))

(defn- clear-activity! [user]
  (jdbc.sql/delete! *db* :activity {:created_by (:user/id user)}))

(deftest test-a-clean-location-deletes
  (tf/testing "POST /location/:id/delete/"
    (fixtures)
    (fn [{:keys [user location]}]
      (let [id (:location/id location)
            sess (app.test/login (:user/email user) "testpassword123")
            {:keys [response] :as sess} (peri/request sess (delete-url location))
            token (test.i/response-anti-forgery-token response)
            {:keys [response]} (peri/request sess (delete-url location)
                                             :request-method :post
                                             :params {:__anti-forgery-token token})]
        (try
          (is (= 303 (:status response)))
          (is (nil? (location.i/get-by-id *db* id)))
          (finally
            (clear-activity! user)))))))

(deftest test-a-location-becomes-deletable-once-its-history-goes
  (tf/testing "delete the material, then the location"
    (assoc (fixtures)
           [::material.i/factory :key/material] {:db *db*
                                                 :accession (ig/ref :key/accession)
                                                 :location (ig/ref :key/location)})
    (fn [{:keys [user location material]}]
      (let [sess (app.test/login (:user/email user) "testpassword123")]
        (try
          (testing "blocked while it holds a plant"
            (let [{:keys [response]} (peri/request sess (delete-url location))
                  body (Jsoup/parse ^String (:body response))]
              (is (nil? (.selectFirst body "form[method=post]")))
              (is (re-find #"1 material" (.text body)))))
          (let [material-url (str "/material/" (:material/id material) "/delete/")
                {:keys [response] :as sess} (peri/request sess material-url)
                token (test.i/response-anti-forgery-token response)]
            (peri/request sess material-url
                          :request-method :post
                          :params {:__anti-forgery-token token}))
          (testing "and deletable once it does not"
            (let [{:keys [response]} (peri/request sess (delete-url location))
                  body (Jsoup/parse ^String (:body response))]
              (is (some? (.selectFirst body "form[method=post]"))
                  "a blocker is a step, not a wall")))
          (finally
            (clear-activity! user)))))))

(deftest test-a-reader-cannot-delete
  (tf/testing "as a reader"
    (fixtures :reader)
    (fn [{:keys [user location]}]
      (let [sess (app.test/login (:user/email user) "testpassword123")
            {:keys [response]} (peri/request sess (delete-url location))]
        (is (= 302 (:status response)))
        (is (some? (location.i/get-by-id *db* (:location/id location))))))))
