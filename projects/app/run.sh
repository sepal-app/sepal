...
#
# Only for a database that already exists. The jar provisions a new one, and
# provision! refuses a path that is already occupied — so touching the file
# here would leave a fresh install with an empty database and no schema.
if [[ -f "$DB_PATH" ]]; then
