    ...
    (let [provisioned-dir (fs/create-temp-dir {:prefix "sepal-schema-parity-provisioned"})
      ...
      (try
        (let [provisioned-path (fresh-db provisioned-dir)
          ...
          (is (= (query provisioned-path "select code || '|' || label from observation_type order by code")
                 (query migrated-path "select code || '|' || label from observation_type order by code"))
              "the observation_type seed in schema.sql must match the one in the migration")
          (is (= (query provisioned-path "select type || '|' || code || '|' || label from observation_value order by type, code")
                 (query migrated-path "select type || '|' || code || '|' || label from observation_value order by type, code"))
              "the observation_value seed in schema.sql must match the one in the migration")
          (is (= (query provisioned-path "select name || '|' || label || '|' || coalesce(clonal, '') from propagation_type order by name")
                 (query migrated-path "select name || '|' || label || '|' || coalesce(clonal, '') from propagation_type order by name"))
              "the propagation_type seed in schema.sql must match the one in the migration")
          (is (= (query provisioned-path "select name || '|' || label from propagation_status order by name")
                 (query migrated-path "select name || '|' || label from propagation_status order by name"))
              "the propagation_status seed in schema.sql must match the one in the migration"))
        (finally
